...
Implementation Home Page URL: https://pine.blog/

Source Code repo URL(s) (optional): 
* [ ] 100% open source implementation

Programming Language(s): pythonn

Developer(s): [Brian Schrader](https://brianschrader.com)

Implementation Classes (Sender and/or Receiver): Sender & Receiver
...
I am currently using the `mf2py` library to parse documents during webmention discovery and theh library incorrectly parses links with no `rel` attribute and joins them to the root url (as is expected in earlier tests). I have decided not to create a different implementation just for this test. I will look into fixing the `mf2py` issue itself.
...
The current nature of my system prefers a hard-delete when dealing with user data. A soft-delete would be required for this functionality. It may be implemented in the future, but not now. 
...
A simple, one sentence message is returned. Most errors return the exact language included in the standard.
